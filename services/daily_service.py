...
def get_daily_data():
    # Ignorar creditos con estatus 'Cancelado' o 'Rechazado'
    noCanceledCredits = db.session.query(Credit).filter(
        ~Credit.estatus_credito.in_(["Cancelado", "Rechazado"])
    ).all()
    
    # Créditos nuevos e ignorar canccelados/rechazados
